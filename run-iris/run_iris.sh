...
: "${SOURCE_PROJECT_KEY?}" "${SOURCE_ORGANIZATION?}"
: "${DESTINATION_PROJECT_KEY?}" "${DESTINATION_ORGANIZATION?}"

# Run IRIS from SQC EU to SQS
function run_iris_next () {
  java \
    -Diris.source.projectKey="$SOURCE_PROJECT_KEY" \
    -Diris.source.organization="$SOURCE_ORGANIZATION" \
...
function run_iris_sqc_us () {
  java \
    ...
    -Diris.source.token="$SONAR_IRIS_SQC_EU_TOKEN" \
    -Diris.destination.projectKey="$DESTINATION_PROJECT_KEY" \
    -Diris.destination.organization="$DESTINATION_ORGANIZATION" \
